...
<h1>Wurzelmodell {{ $propellerModellWurzel->name }} bearbeiten</h1>
<div class="row">
    <div class="col">
        {!! Form::open(['action' => ['PropellerModellWurzelnController@update', $propellerModellWurzel->id], 'method' => 'POST']) !!}
            <div class="form-group row">
                {{Form::label('name','W',['class' => 'col-sm-1 col-form-label'])}}
                <div class="col">
                    <select class="form-control col-sm-1" name="propeller_klasse_geometrie_id">
                        <option value="{{ $propellerModellWurzel->propellerKlasseGeometrie->id }}">{{ $propellerModellWurzel->propellerKlasseGeometrie->name }}</option>
                    ...
                    <select class="form-control col-sm-1" name="propeller_drehrichtung_id">
                        <option value="{{ $propellerModellWurzel->propellerDrehrichtung->id }}">{{ $propellerModellWurzel->propellerDrehrichtung->name }}</option>
                    ...
                    <select class="form-control col-sm-1" name="propeller_modell_kompatibilitaet_id">
                        <option value="{{ $propellerModellWurzel->propellerModellKompatibilitaet->id }}">{{ $propellerModellWurzel->propellerModellKompatibilitaet->name }}</option>
                    ...
                    Winkel
                    {{Form::number('winkel',$propellerModellWurzel->winkel, ['class' => 'form-controll','step' => '0.5','min' => '-10', 'max' => '20', 'placeholder' =>'Bsp.: -5'])}}
                    Konuswinkel
                    {{Form::number('konuswinkel',$propellerModellWurzel->konuswinkel, ['class' => 'form-controll','step' => '1','min' => '0', 'max' => '8', 'placeholder' =>'Bsp.: 4'])}}
                </div>
            </div>
            <div class="form-group row">
                {{Form::label('bereichslaenge','Bereichslänge [mm]',['class' => 'col-sm-1 col-form-label'])}}
                <div class="col-sm-2">
                    {{Form::number('bereichslaenge',$propellerModellWurzel->bereichslaenge, ['class' => 'form-controll','step' => '1', 'min' => '1', 'placeholder' =>'Bsp.: 150'])}}
                </div>
            </div>
            <div class="form-group row">
                {{Form::label('kommentar','Kommentar',['class' => 'col-sm-1 col-form-label'])}}
                <div class="col-sm-2">
                    {{Form::textarea('kommentar',$propellerModellWurzel->kommentar, ['class' => 'form-controll','rows' => 4, 'cols' => 20, 'placeholder' =>'100 Zeichen für Infos'])}}
                </div>
            </div>
